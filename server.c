...
void login(int client_sock){
	...
	//  SAME USER IS NOT ALLOWED ALLOWED FROM MULTIPLE TERMINALS
...
void signup(int client_sock){
	...
	//  IF FILE EMPTY, LOGIN ID STARTS FROM 1
	//  ELSE INCREMENT FROM PREVIOUS VALUE
	if(fp==0){
		u.login_id = 1;
		strcpy(u.name, name);
...
void crud_train(int client_sock){
	...
	if(choice==1){  					// ADD TRAIN
		...
		tdb.total_seats = 10;				// BY DEFAULT WE TAKE 10 SEATS
